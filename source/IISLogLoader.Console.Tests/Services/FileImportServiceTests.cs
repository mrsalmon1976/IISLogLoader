...
namespace IISLogLoader.Console.Tests.Services
{
    ...
    {
        [Test]
        public void ImportLogFile_SavesLogEventsFromFile()
        {
            // setup
            ...
            fileLoadService.LoadFile(filePath).Returns(logEvents);

            // execute
            IFileImportService fileImportService = CreateService(fileLoadService);
            fileImportService.ImportLogFile(logDbContext, filePath).GetAwaiter().GetResult();
